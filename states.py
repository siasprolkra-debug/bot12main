...
class BroadcastManager:
    ...
    def is_active(self, preset_id):
        return preset_id in self.active
    ...
    def update(self, preset_id, sent=0, errors=0):
        if preset_id in self.active:
...
class Database:
    ...
    def update_history(self, history_id, sent, errors, status='completed'):
        cursor = self.conn.cursor()
        cursor.execute(
            "UPDATE history SET sent = ?, errors = ?, end_time = ?, status = ? WHERE id = ?",
    ...
    async def run_broadcast(self, preset_id, account_id):
        ...
        for chat in chats:
            ...
            if time.time() - start_time > duration_seconds:
                print(f"DEBUG: Время вышло")
                break

            success, error = await self.send_message(client, chat[2], message_text)
            if success:
                sent += 1
                broadcast_manager.update(preset_id, sent=1)
                print(f"DEBUG: Отправлено в {chat[3]}")
            else:
                errors += 1
                broadcast_manager.update(preset_id, errors=1)
                print(f"DEBUG: Ошибка отправки в {chat[3]}: {error}")

            await asyncio.sleep(delay)

        db.update_history(history_id, sent, errors, 'completed')
        broadcast_manager.remove(preset_id)
        print(f"DEBUG: Рассылка завершена. Отправлено: {sent}, Ошибок: {errors}")
...
async def main():
    ...
    try:
        ...
        cursor.execute("SELECT delay FROM presets LIMIT 1")
    except:
        print("⚠️ Обновление структуры БД...")
        cursor = db.conn.cursor()
        try:
            cursor.execute("ALTER TABLE presets ADD COLUMN delay INTEGER DEFAULT 5")
        except:
            pass
        try:
            cursor.execute("ALTER TABLE presets ADD COLUMN duration INTEGER DEFAULT 60")
        except:
            pass
        db.conn.commit()
